...
const S = {
  ...
  modeWrap:   { display: "flex", background: "#0c0c0f", border: "1px solid #1e2035", borderRadius: 6, overflow: "hidden", width: "fit-content" },
  modeBtn:    a => ({ padding: "8px 22px", fontSize: 12, letterSpacing: "0.1em", textTransform: "uppercase", border: "none", cursor: "pointer", fontFamily: "inherit", background: a ? "#7b9fff" : "transparent", color: a ? "#0c0c0f" : "#4a5070", fontWeight: a ? 700 : 400, transition: "all 0.15s" }),
  ...
  fileZone:   (a) => ({ border: "1px dashed #2a2f50", borderRadius: 5, padding: 18, textAlign: "center", fontSize: 12, color: a ? "#7b9fff" : "#4a5070", cursor: "pointer", transition: "border-color 0.15s", background: "#0c0c0f", ...(a ? { borderColor: "#3b4a7a" } : {}) }),
...
export default function CSMTTS() {
  const [mode, setMode]           = useState("generate"); // "generate" | "clone"
  ...
  const canGenerate = !loading && text.trim().length > 0 && !charOver &&
    (mode === "generate" || (refAudio && refText.trim()));
    ...
    <div style={S.root}>
      ...
      <div style={S.main}>
        ...
        <div style={S.infoBox}>
          ...
          &nbsp;Voice Clone mode matches any custom voice from a short recording.
        </div>

        {/* Mode */}
        <div style={S.modeWrap}>
          <button style={S.modeBtn(mode === "generate")} onClick={() => setMode("generate")}>Generate</button>
          <button style={S.modeBtn(mode === "clone")}    onClick={() => setMode("clone")}>Voice Clone</button>
        </div>
        ...
        <div style={S.panel}>
          ...
          {mode === "clone" && <>
            <div>
              <label style={S.label}>
                Reference audio
                <span style={{ color: "#2a3050", marginLeft: 6 }}>WAV · 3+ sec recommended</span>
              </label>
              <div style={S.fileZone(!!refAudio)} onClick={() => document.getElementById("refIn").click()}>
                {refAudio ? `✓ ${refAudio.name}` : "Click to upload WAV file"}
                <input id="refIn" type="file" accept="audio/wav,audio/*" style={{ display: "none" }}
                  onChange={e => setRefAudio(e.target.files[0] || null)} />
              </div>
            </div>
            <div>
              <label style={S.label}>Reference transcript</label>
              <textarea style={{ ...S.textarea, minHeight: 60 }} value={refText}
                onChange={e => setRefText(e.target.value)}
                placeholder="Exact words spoken in the reference audio…" spellCheck={false} />
            </div>
          </>}
